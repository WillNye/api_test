...
class DeleteMixin:
    ...
    def test_invalid_id(self):
        response = self.session.delete(f'{self.url}/{self.route}/a01')
        self.assertEqual(response.status_code, 404)

        return response

    def test_id_does_not_exist(self):
        response = self.session.delete(f'{self.url}/{self.route}/{uuid4()}')
        self.assertEqual(response.status_code, 404)

        return response

    def test_unauth(self):
        response = requests.delete(f'{self.url}/{self.route}/{self.object_id}')
...
class ListMixin:
    ...
    def test_filter(self, filter_key: str):
        ...
        response = self.session.get(f'{self.url}/{self.route}', params={filter_key: filter_val})
    ...
    def test_sort(self, sort_param: str, sort_key: str):
        response = self.session.get(f'{self.url}/{self.route}', params={sort_param: sort_key})
        self.assertTrue(response.ok)
